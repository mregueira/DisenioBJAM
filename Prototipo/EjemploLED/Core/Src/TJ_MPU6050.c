...
void MPU6050_Get_Accel_RawData(RawData_Def *rawDef)
{
	...
	if((i2cBuf[1]&&0x01))
	{
		I2C_Read(XA_OFFSET_H_REG, AcceArr,6);

		//Accel Raw Data
		rawDef->x = (((AcceArr[0]<<8) + (AcceArr[1]&0xFE))>>1); // x-Axis
		rawDef->y = (((AcceArr[2]<<8) + (AcceArr[3]&0xFE))>>1); // y-Axis
		rawDef->z = (((AcceArr[4]<<8) + (AcceArr[5]&0xFE))>>1); // z-Axis
		//Gyro Raw Data
